"""
Tests for the GET /json_item/{project_id} endpoint (get_all_json_items).

Test database contains:
  Project "Foo" (public)
    - key "json_first", json_value {"count": 1}

  Project "Bar" (private)
    - key "json_third", json_value {"count": 3}
...
def test_public_project_anonymous_user_sees_all_items(make_client, seeded_data):
    """Anonymous user on a public project receives all JSON items."""
    client = make_client(current_user=None)
    response = client.get(f"/json_item/{seeded_data['foo_id']}")

    assert response.status_code == 200
    keys = [item["key"] for item in response.json()]
    assert "json_first" in keys


def test_public_project_authenticated_user_sees_all_items(make_client, seeded_data):
    """Authenticated user on a public project sees all JSON items."""
...
def test_private_project_anonymous_user_gets_403(make_client, seeded_data):
    ...
    client = make_client(current_user=None)
    response = client.get(f"/json_item/{seeded_data['bar_id']}")

    assert response.status_code == 403


def test_private_project_user_with_project_access_sees_all_items(make_client, seeded_data):
    """User with project access sees all JSON items in a private project."""
    client = make_client(current_user=seeded_data["user_with_access"])
    response = client.get(f"/json_item/{seeded_data['bar_id']}")
